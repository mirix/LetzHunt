...
def crop_image_center(image_path: str, output_path: str, crop_percentage: float = 0.7):
    """
    ...
    Args:
        ...
        crop_percentage: The percentage of the image to keep (e.g., 0.7 for 70%).
...
class OCRFlux_Wrapper:
    ...
    def _correct_date_to_season(self, date_obj: datetime.datetime, season_start: datetime.datetime,
        ...
        return None # Return None if date cannot be placed in the season

    def _parse_ocr_output(self, text: str, lot_number: int) -> Tuple[List[str], Dict[str, Any]]:
        """
        Parses OCR output with a two-tier logic (strict then relaxed).
        """
        debug_info = {
            'raw_text': text,
            'logic_used': 'None',
        ...
        # --- Tier 1: Strict Logic ---
        debug_info['logic_used'] = 'Tier 1 (Strict)'
        lot_pattern = r'\b0*' + f"({lot_number}|{lot_number:03d})" + r'\b'
        lot_match = re.search(lot_pattern, text_clean)

        dates = []
        if lot_match:
            debug_info['lot_match_found'] = True
            text_after_lot = text_clean[lot_match.start():]
            date_pattern = r'\b\d{2}/\d{2}/\d{4}\b'
            found_date_strings = re.findall(date_pattern, text_after_lot)

            parsed_dates = []
            for date_str in found_date_strings:
                date_obj = dateparser.parse(date_str, settings={'DATE_ORDER': 'DMY'})
                if date_obj:
                    debug_info['all_dates_found_raw'].append(date_obj.strftime('%d/%m/%Y'))
            ...
            if parsed_dates:
                parsed_dates.sort()
                dates = sorted(list(set([d.strftime('%d/%m/%Y') for d in parsed_dates])))

        # --- Tier 2: Relaxed Fallback Logic ---
        # If strict logic found no dates, try relaxed logic on the whole (cropped) text.
        if not dates:
            debug_info['logic_used'] = 'Tier 2 (Relaxed Fallback)'
            date_pattern = r'\b\d{2}/\d{2}/\d{4}\b'
            found_date_strings = re.findall(date_pattern, text_clean)
        ...
        debug_info['valid_dates_final'] = dates
        return dates, debug_info

    def recognize_dates(self, image_path: str, lot_number: int) -> Tuple[List[str], str, Dict[str, Any]]:
        """
        ...
        # --- Pre-processing Step: Crop the image ---
        cropped_path = get_cropped_tile_path(lot_number)
        if not crop_image_center(image_path, cropped_path):
        ...
        try:
            ...
            # Parse the output using the two-tier logic
            dates, debug_info = self._parse_ocr_output(raw_text, lot_number)
